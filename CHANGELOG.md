...
- 修复: 播放页 同时开启进度跳过、自动下集、预载下集条件时触发播放下下集 [issue #488]
- 变动: 数据库字段(注意备份)
...
- 特性: 热搜支持欢网
- 优化: 重构后端schema和传参类型
- 优化: 重构aigc模块, 支持不同提供商, 支持联网查询, 支持深度思考(取决模型); 修复结果复制失败(patch)
- 优化: 重构gzip压缩, fflate->pako(faster 40%-50%)
- 优化: 重构terminal组件, 修复多实例搜索冲突
- 优化: 默认ui参数和字体调整
...
- 变动: 插件/环境 请求通道 http(超时机制) -> ipc(无超时)
